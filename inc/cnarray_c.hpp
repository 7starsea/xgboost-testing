...
public:
	...
	cndarray_c<T, N> slice(int start, int length, int axis=0) const{
		if(start < 0) start += shape_[axis];
		if(length <= 0 || start + length >= shape_[axis]){
			throw std::runtime_error("overflow start, length in slice");
		}
		cndarray_c<T, N> sub_cndarr (data_ + start * strides_[axis]);
		for(int i = 0; i < N; ++i){
			if(i == axis){
				sub_cndarr.shape(i) = length;
			}else{
				sub_cndarr.shape(i) = shape_[i];	
			}
			sub_cndarr.strides(i) = strides_[i];
		}
		return sub_cndarr;	
	}
